...
前端读取款式数据时以成员 B 的嵌套结构为准：使用 `item.style_id` 作为唯一编号，使用 `item.image_path` 读取图片，标签从 `item.tags.style`、`item.tags.color`、`item.tags.craft`、`item.tags.scene`、`item.tags.crowd` 读取。现有数据也可能包含 `tags.length`、`tags.texture` 等扩展标签，前端可按需展示。

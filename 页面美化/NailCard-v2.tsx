...
export default function NailCard({
  ...
  // 在分类页面只显示前6个标签，保持界面整洁
  const allTags = tagGroups.flatMap((group) => nailStyle.tags[group] ?? []);
  const displayTags = allTags.slice(0, 6); // 限制显示标签数量
    ...
    <article className="nail-card fade-in-up">
      ...
      <div className="nail-card__body">
        ...
        <div className="nail-card__tags" aria-label="款式标签">
          ...
          {hasMoreTags && (
            <span className="nail-card__more-tags">+{allTags.length - 6}</span>
          )}
        </div>
